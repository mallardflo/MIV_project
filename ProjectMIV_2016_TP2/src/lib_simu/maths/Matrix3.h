    ...
    */
    ...
    public:
        ...
        */
		...
		//-----------------------------------------------------------------------
		inline void Matrix3::GetRow(size_t iRow, Vector3& out_row) const
		{
			  out_row.x = m[iRow][0];
			  out_row.y = m[iRow][1];
			  out_row.z = m[iRow][2];
		}
		inline Real get(int i, int j) { return m[i][j]; }
		//-----------------------------------------------------------------------
		inline void Matrix3::set(	Real m11, Real m21, Real m31,
		...
		inline void Matrix3::set(int i, int j, Real val)
		{
			m[i][j] = val;
		}
		...
		inline Real frobeniusNorm() {
			Real sum = 0.0f;
			for (int i = 0 ; i < 3 ; i++) for (int j = 0 ; j < 3 ; j++) sum += m[i][j]*m[i][j];
			return sum;
		}

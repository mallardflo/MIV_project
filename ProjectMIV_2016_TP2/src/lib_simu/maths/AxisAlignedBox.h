	...
	*/
	...
	public:
		...
		*/
		...
		inline void increase(Maths::Vector3& size) {
			mMinimum -= size;
			mMaximum += size;
		}

		inline void scale_increase(Maths::Real s) {
			Maths::Vector3 size = getSize();
			size *= s;
			increase(size);
		}
		...
		*/
		inline const Vector3* getAllCorners(void) const
		{
			...
			if (!mpCorners)	mpCorners = new Vector3[8];
		...
		*/
		...
		inline void projectPoint(Maths::Vector3& pt) {
			
			if (pt.x < mMinimum.x) pt.x = mMinimum.x;
			if (pt.y < mMinimum.y) pt.y = mMinimum.y;
			if (pt.z < mMinimum.z) pt.z = mMinimum.z;

			if (pt.x > mMaximum.x) pt.x = mMaximum.x;
			if (pt.y > mMaximum.y) pt.y = mMaximum.y;
			if (pt.z > mMaximum.z) pt.z = mMaximum.z;
